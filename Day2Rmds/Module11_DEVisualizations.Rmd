...
<details>
    ...
    res_anno <- res # copy table
    # add a column and assign all genes are non-significance
    res_anno$Call <- rep(FALSE, length(res$baseMean)) 

    # change 'Call' column to TRUE if meets conditions for significant differences
    res_anno[which(!is.na(res_anno$padj) & res_anno$padj < 0.05 & abs(res_anno$log2FoldChange) >= log2(1.5)), ]$Call <- TRUE

    # reorder table to rank significant genes at the top
    res_anno <- res_anno[order(-res_anno$Call),]
    ```
...
How many DE genes would we have if we only apply a threshold to our adjusted pvalues and ignored fold-change?
```{r StatSigGenes}
