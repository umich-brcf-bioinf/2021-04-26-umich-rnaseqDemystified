---
title: "Day 2 - Module XX: DESeq2 Initialization"
...
**TODO: Paper doesn't seem to be published - should we check in with researcher or use a different dataset?**
...
**TODO: Set up why using a pre-made sample sheet (& explain that can use excel/save to csv output)**
Since there are a large number of samples (and to increase the reproduciblity of our code), we generated a sample information table in excel and exported it as a '.csv' file so that it is in a 'plain text' format that can be easily loaded into our R session.

**TODO: Create bump out here**
Creating a csv formatted table
...
**TODO: Edit sample sheet so labels better reflect actual sample information**

We'll load our sample information sheet, `SampleInfo_trimmed.csv`, to unblind our samples.
```{r SampleInfo1}
...
```{r SampleInfo2}
MetaInfo$Gtype.Tx <- factor(MetaInfo$Gtype.Tx, levels = c("ko.noTx", "wt.noTx", "ko.TxB", "wt.TxB")) ## levels = c(Case, Control); Control should be last if only two groups & first if three groups. 
unique(MetaInfo$Gtype.Tx)
```
...
**Checkpoint**: *If you see `dds` in your environment panel, please indicate with the green 'yes' button. Otherwise, please use the 'raise hand' button*
...
> *Note*: More complex questions, including determining if a fold-change due to treatment is different across groups, such as patient samples, "interaction terms" can be included in the design formula, such as outlined in [this support thread](https://support.bioconductor.org/p/98628/).

**TODO: Update to collapsable section**

><details>
>  <summary>*Modeling batch effects with DESeq2*</summary>
...
>   Let's try add some additional meta-data information where we have counfounding batch effects and create another DESeq2 object.
  ...
  #MetaInfo
  MetaInfo$batch <- factor(c(rep(c("Day1"), 3), rep(c("Day2"), 3), rep(c("Day3"), 3), rep(c("Day4"), 3)), levels = c("Day1", "Day2", "Day3", "Day4"))
...
Before moving on with our group comparisons, we'll take a short break before proceeding.
