...
**TODO: Finalize formatting for pop out code**
<details>
...
Here we will filter out any genes that have less than 10 counts across any of the samples. **TODO: May or may not want to keep filtering level so high here since makes rlog normalization a but wonky, even though rlog count normalization figure improves**
```{r PreFilter}
...
We'll also filter the `dds_patient` genes, but we'll be less stringent with our filter for now.
```{r PreFilterPatient}
keep <- rowSums(counts(dds_patient)) >= 1
